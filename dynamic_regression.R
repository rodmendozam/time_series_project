...
##############################
# DYNAMIC REGRESSION AUTOMATIC
##############################
# Model
ener_sweden.auto <- auto.arima(ener_sweden.ts.y_train, xreg=gdp_sweden.ts.x_train)
ener_sweden.auto.fore <- forecast(ener_sweden.auto, h = length(gdp_sweden.ts.x_valid), xreg = gdp_sweden.ts.x_valid)
plot(ener_sweden.auto.fore)

# Denormalize
x_train <- exp(diffinv(ener_sweden.ts.y_train, differences = 1, xi = log(2.700534)))
x_val <- exp(diffinv(ener_sweden.ts.y_valid, differences = 1, xi = log(5.670417)))
x_pred <- exp(diffinv(ener_sweden.auto.fore$mean, differences = 1, xi = log(5.670417)))
x_train <- x_train[1:length(x_train)-1]
x_val <- x_val[1:length(x_val)-1]
x_pred <- x_pred[1:length(x_pred)-1] 
# c.i. 95
x_pred_upper_95 <- exp(diffinv(ener_sweden.auto.fore$upper[,2], differences = 1, xi = log(5.670417)))
x_pred_lower_95 <- exp(diffinv(ener_sweden.auto.fore$lower[,2], differences = 1, xi = log(5.670417)))
x_pred_upper_95 <- x_pred_upper_95[1:length(x_pred_upper_95)-1]
x_pred_lower_95 <- x_pred_lower_95[1:length(x_pred_lower_95)-1]
# c.i. 80
x_pred_upper_80 <- exp(diffinv(ener_sweden.auto.fore$upper[,1], differences = 1, xi = log(5.670417)))
x_pred_lower_80 <- exp(diffinv(ener_sweden.auto.fore$lower[,1], differences = 1, xi = log(5.670417)))
x_pred_upper_80 <- x_pred_upper_80[1:length(x_pred_upper_80)-1]
x_pred_lower_80 <- x_pred_lower_80[1:length(x_pred_lower_80)-1]

# plot by modifying ener_sweden.proxy
ener_sweden.auto.fore$x <- x_train
ener_sweden.auto.fore$mean <- x_pred
ener_sweden.auto.fore$upper[,2] <- x_pred_upper_95
ener_sweden.auto.fore$lower[,2] <- x_pred_lower_95
ener_sweden.auto.fore$upper[,1] <- x_pred_upper_80
ener_sweden.auto.fore$lower[,1] <- x_pred_lower_80
plot.forecast(ener_sweden.auto.fore)

# plot manually (run block)
nb_train <- length(x_train)
nb_val <- length(x_val)
indices <- c(1:(nb_train+nb_val))
x_train_and_nan <- c(x_train, rep(NaN, nb_val))
